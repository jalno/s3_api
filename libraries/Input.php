...
class Input
{
	 ...
	 *
	 * @param   File\Local   $file       A jalno local file
	 ...
	 * @return  Input
	 */
	public static function createFromFile(File\Local $file, ?string $md5sum = null, ?string $sha256sum = null): self
	{
		$input = new Input();
	...
	/**
	 * Set the local jalno file
	 *
	 * @param   File\Local  $file
	 */
	public function setFile(File\Local $file): void
	{
		$this->file = $file;
		$this->data = null;
	}
	 ...
	 *
	 * @param   File\Local  $file  The absolute path to the file for which we want to get the MIME type
	 *
	 * @return  string  The MIME type of the file
	 */
	private function getMimeType(File\Local $file): string
	{
		$type = false;

		// Fileinfo documentation says fileinfo_open() will use the
		// MAGIC env var for the magic file
		if (extension_loaded('fileinfo') and
			isset($_ENV['MAGIC']) and
			($finfo = finfo_open(FILEINFO_MIME, $_ENV['MAGIC'])) !== false
		) {
			if (($type = finfo_file($finfo, $file->getPath())) !== false)
			{
